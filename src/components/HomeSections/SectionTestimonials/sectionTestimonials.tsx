...
export function SectionTestimonials() {
  ...
  const fullTitle = "Veja o que nossos clientes falam da gente";

  useEffect(() => {
  if (isInView) {
    let index = 0;
    const interval = setInterval(() => {
      setAnimatedTitle(fullTitle.substring(0, index + 1));
      index++;
  ...
  return (
    <div ref={sectionRef} className="py-8 px-2 sm:py-14 sm:px-6 md:px-12">
      <h1 className="text-xl sm:text-3xl md:text-5xl font-extrabold text-center py-4 sm:py-8 font-gilroy text-[#223872] mb-6 sm:mb-9 min-h-[60px] sm:min-h-[80px]">
        {animatedTitle}
